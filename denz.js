...
try {
        ...
        mess = {
			...
			only: {
				...
				ownerB: '𝙾𝙽𝙻𝚈 𝙵𝙾𝚁 ī.ᴀᴍ/ꪶ͢ʜᴜsɴɪ sᴇʀꫂ⁩⁩',
		...
		const katalog = (teks) => {
             res = denz.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 2021, "message": teks, "footerText": "*_© ī.ᴀᴍ/ꪶ͢ʜᴜsɴɪ sᴇʀꫂ⁩⁩_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
...
denz.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Sorry it seems at this time ${NamaOwner} currently Offline\n\n*Reason :* ${alasanafk}\n*Since :* ${heheh.hours} Hour, ${heheh.minutes} Minutes, ${heheh.seconds} second\n\nPlease contact again after Online`, "*_© ī.ᴀᴍ/ꪶ͢ʜᴜsɴɪ sᴇʀꫂ⁩⁩_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
		...
		const reply = (teks) => {
			denz.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `ī.ᴀᴍ/ꪶ͢ʜᴜsɴɪ sᴇʀꫂ⁩`,body:"",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/917025868709`}}})
		}
		const reply2 = (teks) => {
...
☯ 𝙳𝙰𝚈 : ${week} ${weton}
☯ 𝙳𝙰𝚃𝙴 : ${calender}`
sendButLocation(from, `${menu}`, "ī.ᴀᴍ/ꪶ͢ʜᴜsɴɪ sᴇʀꫂ⁩⁩", {jpegThumbnail:gambar,name:""}, [
{buttonId:`allmenu`,buttonText:{displayText:'😱SHOW MENU'},type:1},
{buttonId:`owner`,buttonText:{displayText:'😎OWNER'},type:1},
{buttonId:`script`,buttonText:{displayText:'📃SCRIPT'},type:1}
...
│
└─────────────────❒`
sendButLocation(from, `${menu}`, "ī.ᴀᴍ/ꪶ͢ʜᴜsɴɪ sᴇʀꫂ⁩⁩", {jpegThumbnail:gambar,name:""}, [{buttonId:'owner',buttonText:{displayText:'OWNER'},type:1},{buttonId:'script',buttonText:{displayText:'SCRIPT'},type:1}], {contextInfo: { mentionedJid: [otod]}})
break
case 'menu':
 stod = `${sender}`
 listMsg = {
 buttonText: '𝙻𝙸𝚂𝚃 𝙼𝙴𝙽𝚄⁩',
 footerText: '*© ʜᴜsɴɪ sɪʀ*',
 description: `╭────────────────
┊     𝙷𝙸 𝙱𝚁𝙾 @${stod.split('@')[0]} 
┊ 
┊
